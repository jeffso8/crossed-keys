...
export default function Grid(props) {
  ...
  const {gameOver, user, redTurn, roomID, redScore, handleRedScoreChange, handleBlueScoreChange, blueScore} = props;
    ...
    return (
      <div className={className} style={style.columns}>
        {rowColor.map((color, index) => {
          ...
          const randDeg = (Math.random() + 0.09) * (Math.round(Math.random()) ? 1 : -1);
