    ...
    io.on('connection', (socket) => {
        ...
        socket.on('updateTurn', (data) => {
          ...
          Rooms.findOne({roomID: data.roomID}, function(err, res) {
            ...
            res.save();
            socket.nsp.in(data.roomID).emit('redTurn', {redTurn: res.isRedTurn, turnStartedAt: res.turnStartedAt});
          });
        });
        ...
        socket.on('hostStartGame', (data) => {
          ...
          socket.roomID = data.roomID;
          Rooms.findOneAndUpdate({roomID: data.roomID}, {$set : {colors: colorSorted, words: words, clicked: clicked, isRedTurn: true, redScore: 8, blueScore: 8, gameOver: false, hints:[], turnStartedAt: new Date()}},
